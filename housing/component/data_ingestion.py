...
def download_housing_data(self)->str:
    try:
        ...
        logging.info(f"file :[{tgz_file_path}] has been download successfull.")
...
def extract_tgz_file(self):
    try:
        ...
        logging.info(f"extracting tgz file: [{tgz_file_path}] info dir: [{raw_data_dir}]")
...
def split_data_as_train_test(self)->DataIngestionArtifact:
    try:
        ...
        # Specify  how many split we want to create
        split = StratifiedShuffleSplit(n_splits=1,test_size=0.2,random_state=42)
        for train_index,test_index in split.split(housing_data_frame,housing_data_frame["income_cat"]):
            strat_train_set = housing_data_frame.loc[train_index].drop(["income_cat"],axis=1)
        train_file_path = os.path.join(self.data_ingestion_config.ingested_train_dir,
                                       file_name)
        test_file_path = os.path.join(self.data_ingestion_config.ingestion_test_dir,
                                      file_name)
